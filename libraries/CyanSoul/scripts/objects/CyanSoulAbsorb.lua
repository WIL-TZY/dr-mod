local Absorb, super = Class(Object)

function Absorb:init(x, y, angle)
    super:init(self, x, y)
...
function Absorb:update()
    ...
    local bullets = Utils.filter(Game.stage:getObjects(Bullet), function(v)
        ...
        return v.onCyanAbsorb
    end)
    for _,bullet in ipairs(bullets) do
        if self:collidesWith(bullet) then
            self.hit_bullets[bullet] = true
            local result, result_big = bullet:onCyanAbsorb(self, self.damage)
            local real_result
            if self.big then
...
function Absorb:setSprite(sprite)
    if self.sprite then
        self.sprite:remove()
    end
