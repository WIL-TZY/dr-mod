...
function PhaseEncounter:getNextWaves()
    if self.next_wave and self.next_wave.wave then
        local wave = self.next_wave.wave
        if self.wave_path then
            wave = self.wave_path .. "/" .. wave
        end
        self.next_wave.enemy.selected_wave = wave
        return {wave}
    end
    return {}
end
...
function PhaseEncounter:getEncounterText()
    ...
    if self.current_phase_turn <= #self.phases[self.current_phase] and self.phases[self.current_phase][self.current_phase_turn] then
...
function PhaseEncounter:setNextTurnData()
    ...
    elseif self.current_phase_turn <= #self.phases[self.current_phase] and self.phases[self.current_phase][self.current_phase_turn] then
...
function PhaseEncounter:setWaveOverride(wave, enemy)
    ...
    if type(wave) == "table" then
        self.next_wave = Utils.copy(wave)
        if wave.dialogue then self:setDialogueOverride(wave.dialogue) end
    else
        self.next_wave.wave = wave
    end
    if enemy then
        if type(enemy) == "string" then
            enemy = Game.battle:getEnemyBattler(enemy)
        end
        self.next_wave.enemy = enemy
    elseif not self.next_wave.enemy then
        self.next_wave.enemy = Utils.pick(Game.battle:getActiveEnemies())
    end
end
