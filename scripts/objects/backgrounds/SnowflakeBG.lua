...
function BG:drawBack()
	...
	for x = -100, 740, 50 do
		for y = -100, 580, 50 do
			neodarkbg = Assets.getTexture("bg/snowflake")
			love.graphics.draw(neodarkbg, x + self.offset/2, y + self.offset/2 + 10)
		end
	end
end
...
function BG:drawFront()
	...
	for x = 0, 740, 50 do
		for y = 0, 580, 50 do
			neodarkbg = Assets.getTexture("bg/snowflake")
			love.graphics.draw(neodarkbg, x - self.offset, y - self.offset)
		end
	end
end
